...
class TableViewController: UITableViewController {
    ...
    private var scheduledNotification_cell:[String] = ["Add Scheduled Notification"]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if sections[currentSection] == "SCHEDULED TRIP NOTIFICATIONS" {
            ...
            let buttonView = UIButton(frame: CGRect(x: 100, y: 100, width: 100, height: 50))
            buttonView.backgroundColor = .green
            buttonView.setTitle("Test", for: .normal)
            buttonView.setTitleColor(.black, for: .normal)
            buttonView.tag = indexPath.row
            buttonView.addTarget(self, action: #selector(self.switchChanged(_:)), for: .valueChanged)
